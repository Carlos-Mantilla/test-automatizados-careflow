...
const categoryStats = testQuestionsData.reduce(
    (acc, item) => {
        const category = item.category; // p.ej. "KnowledgeGeneral"
        const prefix = item.id.split("-")[0]; // p.ej. "KG"
        if (!acc[category]) {
            acc[category] = { count: 0, prefixes: new Set<string>() };
        }
        acc[category].count += 1;
        acc[category].prefixes.add(prefix);
        return acc;
    },
    {} as Record<string, { count: number; prefixes: Set<string> }>
);

// Líneas del tooltip (una por categoría). Ej: "KG (KnowledgeGeneral): 17"
const tooltipLines: string[] = Object.entries(categoryStats).map(
    ([category, info]) => `${Array.from(info.prefixes).join("/")} = ${category}: ${info.count}`
);
